...
public class SuggestionEngine extends Java8BaseListener {
	...
	public void enterMethodDeclaration(
		...
		String methodName=ctx.methodHeader().getText(); //get method name
		String info= ctx.getText(); //get all method declaration info
		String sub=info.substring(0,info.indexOf(methodName));// public/private
		Boolean publicInfo=sub.contains("public");
		if(publicInfo){
			String method=ctx.methodHeader().methodDeclarator().Identifier().getText();
			mMethods.add(method); //add if public
		}
	...
	private TreeSet<Candidate> getTopKNeighbor(String word, int K) {
		...
		for(int i=0; i<mMethods.size(); i++){
			String methodName=mMethods.get(i);
			double distance = Levenshtein.distance(word, methodName);
			tmp.add(new Candidate(methodName,distance)); //add all methods to tmp
		}

		{
			for(int i =0; i<K; i++){ //get only k elements from tmp and add it to minheap
				if(tmp.size()==0){ //if tmp is empty do not add
					System.out.println("THERE SHOULD BE AT LEAST K METHODS");
				}
				else{ //otherwise add candidate to minheap
					minHeap.add(tmp.pollFirst()); //adding candidate of min distance
				}
